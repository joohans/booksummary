...
## ⚡ 다음 작업 세션 체크리스트 (2026-09-24 기준)

> 🔴 **미완 작업 1건 — 「토끼전」 렌더 완료 후 업로드 대기.** 이어받는 순서:
> 1. GPU150 렌더 확인 — `ssh -i ~/.ssh/aws-jsong.pem jsong@192.168.0.150 'pgrep -f "[1]0_create_video" || echo 종료; tr "\r" "\n" < booksummary_render/render_tokki.log | grep -a "제작 완료"'`
>    (⚠️ `pgrep -f` 는 패턴 첫 글자를 대괄호로 감쌀 것 — 안 그러면 자기 명령줄을 잡는다)
> 2. 회수·검증 — `rsync` 로 `booksummary_render/output/Tale_of_the_Rabbit_kr.mp4` → `output/`, md5 양쪽 대조, `ffmpeg -v error -i ... -f null -` 오류 0건
> 3. ⛔ **업로드는 사용자 승인 후** — `.venv/bin/python src/09_upload_from_metadata.py --privacy private --auto --metadata-files output/Tale_of_the_Rabbit_kr.metadata.json`
>    (메타데이터·썸네일 준비 완료. 예약 **12/24(목) 18:00 KST** = `2026-12-24T09:00:00Z`)
> 4. 업로드 후 재생목록 `PLrZ4eky3zPhJkKccHEIMN2bQPNPzFPzxT` 에 `playlistItems().insert` · 썸네일 실물 확인 ·
>    아래 「2026-09-24 (5)」 항목을 완료 기록으로 전환 · 채널 현황(499편·꼬리 12/24·다음 슬롯 12/27) 갱신
> - 다음 편 후보: 금방울전(180) · 유충렬전(171) · 조웅전(171). ⏸ 사씨남정기 예외 착수는 사용자 판단 대기
...
### 채널 현황
- 채널 영상 **498편**. 예약 큐 **30편**, 꼬리 **12/21 정수정전**. 다음 빈 슬롯은 **12/24**(토끼전 렌더 중)
...
2. **2026-09-21 재공급 완료 — 1순위 「박씨전」**(받을 수 있는 수요 644).
   ...
   ~~만복사저포기~~ → **9/24 제작 완료**(`1qg5k1ZlqA4`, 12/18). ~~정수정전~~ → **9/24 제작 완료**(`DlWPiyGZ0nk`, 12/21). ~~토끼전~~ → 9/24 렌더 중(12/24 예정).
   ★ 다음 편 후보: 토끼전(228, ⚠️묶음편 있음) · 금방울전(180) · 유충렬전(171) · 조웅전(171).
...
## 2026-09-24 (5) — 「토끼전」(작자 미상) 제작 중 — 렌더 진행 중

⏳ **업로드 전 상태.** 요약 TTS 4:37 · NLM 8:10 · 렌더 12:50 진행 중. 공개 예약 예정 **12/24(목) 18:00 KST**.
