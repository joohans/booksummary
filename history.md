...
## ⚡ 다음 작업 세션 체크리스트 (2026-09-26 기준)

> ✅ **진행 중 작업 없음.** 「금방울전」 9/26 업로드 완료(`wbXr6yPuZiE`, 12/27 공개) — 고전소설 축 9편째.
> - 다음 편 후보: 유충렬전(171) · 조웅전(171). ⏸ 사씨남정기 예외 착수는 사용자 판단 대기
> - ⛔ 미리 쌓지 않는다 — 착수는 사용자 지시가 있을 때

### 판정 끝난 것 — 다시 손대지 말 것
- ⛔ **고정 댓글 수동 고정 중단** (9/11 판독) — 15일간 고정 5편 vs 미고정 4편 차이가
  **좋아요 1건**. 댓글은 16:10 cron 으로 계속 달되 Studio 수동 고정은 하지 않는다
- ⛔ **채널 전수 썸네일 점검 시도 금지** — 460편에 두 번 시도해 둘 다 오탐(35편→7편, 전부 정상).
  시기별 스타일이 달라 자동 판정이 안 된다. 9/08 누락은 새 기준 8편 한정이었고 고쳤다
- ⛔ **백카탈로그 되살리기** (8/26 검증) → [[project_selection_criteria]] 7번

### 🔴 최우선 — 선정 지표를 수요 축으로 바꿨다 (2026-09-08~09)

**경쟁 중간값 단독은 성과를 예측하지 못한다. 독립된 두 표본에서 재현됐다.**

| 지표 / 표본 | n | Spearman ρ |
|---|---:|---:|
| 경쟁 — 기존 검증군 | 10 | +0.018 |
| 경쟁 — 성숙 표본(152~218일) | 20 | **−0.171** |
| **수요 — 같은 성숙 표본** | 20 | **+0.338** |
| 수요 ÷ 경쟁 | 20 | −0.047 |

- ★ **판정 규칙: 수요 ≥ 300회/월 AND 경쟁 < 5,000. 그 안에서 수요/경쟁 비율로 순위.**
  측정은 `scripts/measure_search_demand.py --provider naver` (네이버 키 연결 완료)
- ⛔ 비율만으로 순위를 매기지 말 것 — 경쟁 0인 책이 최상위로 올라가는데 실제로는 무검색이다
- ⚠️ 수요 ρ=+0.338 도 n=20 에서 **유의하지 않다(p≈0.15)**. 12월에 재검증한다
- ★ **새 기준이 옳았다는 정량 증거가 나왔다** — 옛 기준 최고작(특이점 0.66)보다 좋은 자리에
  새 기준 9편 중 5편이 있다. **수난이대는 수요 5,190 / 경쟁 747 = 6.95** (9/14 공개)
- 상세는 아래 「2026-09-09」·「2026-09-08」 섹션

### 채널 현황
- 채널 영상 **500편**. 예약 큐 **32편**, 꼬리 **12/27 금방울전**. 다음 빈 슬롯은 **12/30**
...
| **11/15** | **원미동 사람들 (양귀자)** | **2,411** | `IAGPFO7WxoU` | 예약 |
| **11/18** | **당신들의 천국 (이청준)** | **835** | `72vRukqX0Cw` | 예약 |
| **11/21** | **침묵의 봄 (레이첼 카슨)** | 3,833~8,841 | `lVVu_nlKd5Q` | 예약 |
| **11/24** | **운영전 (작자 미상)** | **2,134** | `0eWjThwdbd4` | 예약 |
| **11/27** | **순이 삼촌 (현기영)** | 1,947~3,319 | `AU6rHiUO1gk` | 예약 |
| **11/30** | **양반전 (박지원)** | 2,097~4,000 | `iyR04wtaFYE` | 예약 |
| **12/03** | **독 짓는 늙은이 (황순원)** | 1,306~1,423 | `uA7G9R_f6vA` | 예약 |
| **12/06** | **박씨전 (작자 미상)** | 2,745~2,902 | `0Hx7sXbE15A` | 예약 |
| **12/09** | **옹고집전 (작자 미상)** | 1,062~1,384 | `HT8LTkdY0Xg` | 예약 |
| **12/12** | **최척전 (조위한)** | 1,613~1,917 | `aaNcU303Vpc` | 예약 |
| **12/15** | **홍계월전 (작자 미상)** | 1,646 | `1vrHBnsBR6Y` | 예약 |
| **12/18** | **만복사저포기 (김시습)** | 1,037 | `1qg5k1ZlqA4` | 예약 |
| **12/21** | **정수정전 (작자 미상)** | 537 | `DlWPiyGZ0nk` | 예약 |
| **12/24** | **토끼전 (작자 미상)** | — | `PN3T6uT3hXA` | 예약 |
| **12/27** | **금방울전 (작자 미상)** | 2,000~2,188 | `wbXr6yPuZiE` | 예약 |

### 다음 우선 실행 액션
1. ⛔ **미리 쌓지 않는다** — 기본은 대기. 공개 반응(9/07부터 순차)을 보고 판단
2. **2026-09-21 재공급 완료 — 1순위 「박씨전」**(받을 수 있는 수요 644).
   ~~2위 옹고집전~~ → **9/22 제작 완료**(`HT8LTkdY0Xg`, 12/09 공개).
   ~~3위 최척전~~ → **9/23 제작 완료**(`aaNcU303Vpc`, 12/12 공개).
   ~~남은 후보는 홍계월전 하나뿐~~ → **9/24 고전소설 2차 재공급 완료**(47건 → 안정 통과 8건 + ⚠️3건).
   ~~다음 편 1순위 홍계월전~~ → **9/24 제작 완료**(`1vrHBnsBR6Y`, 12/15 공개).
   ~~만복사저포기~~ → **9/24 제작 완료**(`1qg5k1ZlqA4`, 12/18). ~~정수정전~~ → **9/24 제작 완료**(`DlWPiyGZ0nk`, 12/21). ~~토끼전~~ → **9/26 업로드 완료**(`PN3T6uT3hXA`, 12/24).
   ~~금방울전~~ → **9/26 업로드 완료**(`wbXr6yPuZiE`, 12/27).
   ★ 다음 편 후보: 유충렬전(171) · 조웅전(171).
   ⏸ **사씨남정기(1,145)는 6회 중 1회 초과** — 원미동 사람들 같은 예외 착수 여부는 사용자 판단.
   ⚠️ 이번 풀은 9/21 풀보다 한 단계 약하다(유명작=경쟁, 꼬리=수요에 막힘). 상세 `data/candidates.md` 상단
   ★**고전소설 축이 근현대 축보다 낫다**(아래 9/21 참조). 다음 재공급은
   유충렬전·장끼전·토끼전·옥루몽·심청전 계열을 먼저 판다
3. 옛 후보는 `data/candidates.md` **상단 표**(2026-09-09 수요×공급 재정렬).
   ~~1순위 원미동 사람들~~ → **9/12 제작 완료**(`IAGPFO7WxoU`, 11/15 공개).
   ~~당신들의 천국~~ → **9/12 제작 완료**(`72vRukqX0Cw`, 11/18 공개).
   ⛔ ~~사평역~~ **탈락** — 「사평역 소설」이 월 55회다. 3,670 의 대부분은 **9호선 사평역**(지하철).
   ~~침묵의 봄~~ → **9/12 제작 완료**(`lVVu_nlKd5Q`, 11/21 공개).
   ⛔ ~~공정하다는 착각~~ **탈락**(6회 중 5회 게이트 초과, 중간값 10,254 — 차이나는 클라스·인강이 상위).
   ~~운영전~~ → **9/13 제작 완료**(`0eWjThwdbd4`, 11/24 공개).
   **9/13 풀 재공급으로 확보한 남은 후보: 양반전(1,680/3,994) · 순이 삼촌(1,370/2,726~3,316) ·
   독 짓는 늙은이(520/1,305) · 모래톱 이야기(370/969~1,006).** 꺼삐딴 리는 12월까지 보류
   ★ **착수 전 수요와 공급을 함께 잰다** — 공급만 보면 「논 이야기」(수요 140회/월)류를 올린다
3. **선정 기준 판독은 12월 초** (새 기준 9편이 9/07~10/02 공개 → +90일).
   그때 판정 규칙(수요 ≥ 300 / 경쟁 < 5,000)도 함께 재검증한다
4. 네이버 API 는 `.env` 에 연결돼 있다. ⚠️ 광고 UI 는 `ads.naver.com` 광고주센터 →
   좌측에서 **「검색 광고」로 전환** 후 「도구 > API 사용 관리」 (예전 상단 메뉴바 아님)
...
## 2026-09-26 — 「금방울전」(작자 미상) 제작·업로드, 고전소설 축 9편째

`wbXr6yPuZiE`, **12/27(일) 18:00 KST 예약**. 태그 26개 · 썸네일 실물 확인 · 재생목록 추가(position 153). 요약 TTS **4:34**(2,051자, nova) · NLM **11:08**(13분 45초 생성) · 렌더 **15:44 / 478MB**(md5 `fa20f050` 대조, 디코드 오류 0건, 요약 구간 1fps 검사 어두운 연속 최대 2초).
2차 재공급 4순위(받을 수 있는 수요 180). 499편 순회로 중복 없음(「금방울」·「금령」·「Golden Bell」 0건).

- 소스: ko 위키 「금방울전」(13.2KB, ★순환 구조 세 유형) + encykorea `E0007762` + **다빈치맵 현대어 전문 1·2권**(한글 25,660자, 옛한글 0)
- ⛔ 위키문헌 경판 28장본은 옛한글(자모 7,624개)이라 제외. en 위키는 단독 문서가 없다
- 영문 제목 **The Tale of the Golden Bell**(토끼전처럼 뜻을 옮겼다 — 「금방울」은 고유명사라기보다 사물 이름)
- ★ 요약 각도: **진짜 주인공은 금방울** — 해룡은 거의 모든 위기를 제 힘이 아니라 방울의 힘으로 넘긴다(ko 위키 구조론·encykorea 여성주의 독법)
- 이미지 **77장**(130장 중 53장 제외, 41%). `horses galloping dust` 가 목장·경마장·현대 기수를, `snow village` 가 유럽·일본 마을 야경을 불렀다.
  같은 사진이 두 키워드로 **중복 5쌍** 들어왔다. 호랑이·안개 숲·설경 한옥·기와는 통과율이 높았다

### ⚠️ NLM UI 문구 변경 — 「새로 만들기」 → 「새 노트북」

첫 실행이 「새 노트북 버튼 없음」으로 실패했다. 스크린샷에는 로그인·버튼이 정상이었다.
버튼 문구가 **「+ 새 노트북」** 으로 바뀌어 페이지 준비 감지와 클릭 키워드가 모두 빗나갔다 →
`scripts/notebooklm_automator.py` 두 곳에 「새 노트북」 추가, 재실행 한 번에 통과

### 썸네일 — 금방울이 과일·달로 그려진다

- `round glowing golden orb` 는 **꼭지 달린 유자·배**처럼 나왔다 → `perfectly smooth ... no stem and no leaf`
- 그러자 방울이 **하늘로 올라가 달**이 됐다(4장 중 4장) → `resting ON the snow in the foreground ... NOT in the sky` + `There is NO moon and NO sun`
- 여백을 「빈 밤하늘」로 두면 한자 제발이 붙는다 → 왼쪽을 **어두운 설경 소나무 숲**으로 채움
- ★ 낙관 제거는 **확산 채움**(주변 픽셀 평균 반복)으로 — 위쪽 줄의 중앙값으로 메우면 **회색 사각형**이 남는다(1차 실패)

---

## 2026-09-24 (5) — 「토끼전」(작자 미상) 제작·업로드(9/26), 고전소설 축 8편째

`PN3T6uT3hXA`, **12/24(목) 18:00 KST 예약**. 요약 TTS 4:37 · NLM 8:10 · 렌더 **12:50 / 470MB**(md5 `43ddf64c` 대조, 디코드 오류 0건).
태그 26개 · 썸네일 실물 확인(훅·제목·저자 정상) · 재생목록 추가. 업로드는 9/26 사용자 승인 후.
2차 재공급 3순위(받을 수 있는 수요 228). ★ 채널의 「[일당백] 춘향전 토끼전 심청전」(`0tDnOpU_4Vs`) 묶음편과는 **형식이 다른 단독 편** —
재제작 규칙대로 **착수 전 사용자 확인**(9/24 「단독 편으로 제작」).

- 소스: ko 위키 「토끼전」(11.3KB, ★관극시 두 편 — 이유원=토끼 편 / 송만재=자라 편) + encykorea `E0059167`(결말 3유형)
- ⛔ ko 위키 「수궁가」(18.7KB)·en 「Sugungga」 제외 — 판소리 항목이라 창법·유파 개론으로 흐른다
- ⚠️ encykorea 요약문에 독수리·그물 위기를 **자라**가 겪는 것처럼 읽히는 문장이 있다 — ko 위키는 **토끼**의 위기. 토끼가 맞다
- 영문 제목은 기존 매핑 `Tale of the Rabbit` 을 그대로 썼다(채널 관례 `The Tale of ○○` 과 다름 — 기존 매핑 존중)
- 이미지 100장(130장 중 30장 제외). `coral reef fish` 가 열대어·아쿠아리움을, `fishing net rope` 가 동남아 어부를 불렀다.
  **평균 밝기 4.0 짜리 달밤 컷은 σ 가 높아도 화면 대부분이 검정이라 제외** — 내용이 보이는가가 기준
- 썸네일 우하단 낙관은 **처음 잡은 좌표가 빗나갔다**(육안 추정 x 1790~1862 → 실제 1800~1875, y 946~1036).
  ★ **낙관 좌표는 눈대중 말고 붉은 화소 탐지로 잡을 것** — `(r-g>40)&(r-b>40)` 로 경계를 구하고 여백 10px

---

## 2026-09-24 (4) — 「정수정전」(작자 미상) 제작·업로드, 고전소설 축 7편째

`DlWPiyGZ0nk`, **12/21(월) 18:00 KST 예약**. 요약 TTS 4:35 · NLM 9:27 · 렌더 **14:05 / 481MB**(md5 `f965ed54` 대조, 디코드 오류 0건).
이미지 97장(130장 중 33장 제외) · 태그 26개 · 썸네일 확인 · 재생목록 추가(position 151). 2차 재공급 2순위(받을 수 있는 수요 275). 496편 순회로 중복 없음.

### ★★ ko·en 위키가 모두 없는 첫 고전소설 — 현대어 전문을 다빈치맵에서 찾았다

- 소스: encykorea `E0050459` + **다빈치맵 현대어 전문**(한글 33,821자, 옛한글 0) + KCI 전기화(2018) 「여성 인물의 행위자성」 초록
- ⛔ 위키문헌 판본은 옛한글(자모 4,560개)이라 제외. ★ **옛한글 판본만 있을 때는 다빈치맵(`davincimap.co.kr`)을 먼저 볼 것**

### ⚠️ 검색 요약이 홍계월전과 섞였다 — 원문으로 확인해서 잡았다

웹 검색 요약은 「영춘을 처형한다」고 했지만 원문은 **곤장 스무 대**(「주군의 낯을 보아 약간 경책하노라」)다.
영춘의 목을 베는 쪽은 홍계월전이다. 두 작품은 뼈대가 같아서(여성 대원수·남편이 부하·애첩 영춘) 요약 도구가 섞는다.
- ★ **같은 유형의 작품을 연달아 만들 때는 장면을 원문으로 대조할 것**
- 원문 대조로 확인한 차이: 정체가 드러나자 **직책 박탈**(홍계월은 유지) · 장연의 아내는 **셋**(원부인·공주·수정) ·
  두 번째 전쟁에서 **남편을 중군으로 불러 군량 지연으로 곤장** · 결말은 75세 부부의 **백일승천**

### 썸네일 — `dujeonggap` 지정으로 사무라이풍을 걷어냈다

`lamellar armor` 만 쓰자 어깨 갑옷·칼이 일본 사무라이풍으로 나왔다. `Joseon dynasty dujeonggap armor ... studded with rows of
brass rivets ... NOT a curved katana, NOT samurai armor` 로 바꾸자 조선 장수 복식이 나왔고 구도도 처음부터 오른쪽에 섰다.

---

## 2026-09-24 (3) — 「만복사저포기」(김시습) 제작·업로드, 고전소설 축 6편째

`1qg5k1ZlqA4`, **12/18(금) 18:00 KST 예약**. 요약 TTS 4:51 · NLM 7:33 · 렌더 **12:27 / 453MB**(md5 `7b5a0d2f` 대조, 디코드 오류 0건).
이미지 86장 · 태그 26개 · 썸네일 검사 통과 · 재생목록 추가(position 150).
2차 재공급 1순위(받을 수 있는 수요 305). 496편 순회로 중복 없음(「금오신화」·「김시습」도 0건).

### ★ 고전소설 중 처음으로 위키문헌 현대어 번역 전문을 소스로 썼다

- 소스: encykorea `E0017603` + **위키문헌 현대어 번역 전문**(한글 8,046자 / 한자 100자) + ko 위키(818B 토막글)
- ⛔ ko 위키 「김시습」(12,530B, 작품의 **15배**) 제외 — 작가론 드리프트(당신들의 천국 규칙)
- ⛔ en 위키 「Kŭmo sinhwa」 제외 — 만복사저포기 몫이 **20%**뿐, 수록작 다섯 편 개론으로 흐른다
- 영문 제목 **The Tale of Manboksa Temple**(채널 관례. en 위키는 「Account of a chŏp'o game at Manbok Temple」)

### 썸네일 — 좌우 반전으로 구도 문제를 풀었다

Flux 가 「RIGHT THIRD」 지시를 세 번 연속 무시했다. 가장 좋은 그림(seed 692036363)은 달과 선비가 왼쪽이라
훅 상자에 덮였다 → **같은 seed 로 재생성해 좌우 반전**. 달·선비가 오른쪽으로 가고 낙관 두 개가
모두 흰 안개 위로 와서 지우기도 쉬워졌다. ★ **구도가 틀린 명작은 버리지 말고 뒤집어 볼 것**

### 이미지 — 140장 중 54장 제외(39%)

`korean temple` 계열이 **태국 사원·중국 천단·일본 지장보살·일본 신사 등롱**을 끌어왔다(절 소재는 동아시아 공통).
`full moon spring night` 은 판타지 합성 사진을, `hanji paper window light` 는 도심 야경·불꽃놀이를 불렀다.
배꽃·흰 꽃·안개 숲·눈 덮인 산사는 통과율이 높았다.

---

## 2026-09-24 (2) — 「홍계월전」 제작·업로드, 고전소설 축 5편째

`1vrHBnsBR6Y`, **12/15(화) 18:00 KST 예약**. 9/21 풀의 마지막 후보(받을 수 있는 수요 440). 495편 순회로 중복 없음.

- 영문 제목 **The Tale of Hong Gyewol**(en 위키 표제). 이칭 「홍평국전」도 매핑
- Summary 2,056자 → TTS **4:37** · NLM **8:39** · 렌더 **13:19 / 406MB**(md5 `b62346ff` 대조, 디코드 오류 0건)
- 이미지 122장 · 태그 26개 · 썸네일 검사 통과 · 재생목록 추가(position 149)

### ★ ko 위키에 문서가 없는 두 번째 고전소설 — 소스 2개로 갔다

최척전에 이어 「홍계월전」도 ko 위키 문서가 없다. encykorea `E0063966` + en 위키 `Hong Gyewol jeon`(9.1KB) **2개**(운영전 선례).
- ★ **덜 알려진 고전소설은 ko 위키가 없는 게 기본값이다.** 착수 전 확인은 en 위키·encykorea 부터 한다
- ⛔ 「여성영웅소설」·「군담소설」 장르 항목 제외(장르 개론 드리프트). ⛔ 나무위키는 크롤링 차단 위험으로 제외
- 작자 미상이라 작가론 드리프트가 구조적으로 없다

### ⚠️ 이미지 삭제율 41% — 역대 최고 (210장 중 86장 제외)

1차 130장 중 **72장**을 뺐고 보충 80장 중 14장을 더 뺐다.
- ⛔ **`candlelight dark traditional` 은 교회 촛불·핼러윈·현대 인테리어를 부른다**(15장 이상 전멸)
- ⛔ **`korean silk fabric texture` 는 기모노와 향신료 시장을 부른다**
- ⛔ **`lone island calm sea` 는 열대 섬·몰디브를 부른다.** `korean` 을 붙여도 소용없다
- ⛔ `korean royal guard gate ceremony` 는 **영국 근위병·코펜하겐 궁전**까지 끌어왔다
- ★ 보충 라운드 키워드(`palace roof eaves dancheong`·`hanok roof tiles`·`snow hanok village`·`bamboo forest korea`)는 통과율이 높았다 — **건축 부분·계절 풍경이 인물·소품보다 안전하다**
- **두부 요리 사진이 세 편 연속 들어왔다**(독 짓는 늙은이·최척전·홍계월전). `traditional` 계열 키워드의 고질이다
- ⚠️ 확대 검수에서 **편액 한자(勤政殿·明倫堂)**를 두 장 잡았다 — 궁궐 사진은 현판 글자를 품고 있다
- 보충분이 파일명 충돌로 덮어쓰지 않게 합격분을 먼저 옮겨 두고, 되돌릴 때 `mood_k_*` 로 이름을 바꿨다
  (⚠️ `k_mood_*` 로 두면 렌더러의 `mood_*.jpg` glob 에 안 걸린다)

### 썸네일 — 문인화 지시가 화제(畫題) 글씨를 부른다

`literati ink painting in the style of Jeong Seon and Kim Hongdo` 로 두 번 생성했는데 **두 번 다 우상단에
한자 세로글씨**가 생겼다(최척전 때는 낙관만 생겼다). 문인화는 원래 여백에 화제를 써넣는 양식이라,
**여백이 넓은 구도일수록 글씨를 끌어온다.**
- 해결: `Korean sumukhwa ink wash ... no inscription, no poem, no calligraphy column` +
  **인물을 `RIGHT THIRD` 에 고정하고 좌측 여백을 안개로 채움** → 글씨가 2자로 줄었다
- ★ 이번엔 처음부터 **배경만 받아(overlay 전)** 한자 2자와 낙관을 지우고 `overlay_text()` 를 불렀다 — 한 번에 끝났다
- 인물 성별은 `clearly feminine slender figure, long black hair tied high` 로 첫 장부터 읽혔다(박씨전 교훈 적용)

### ⚠️ `pgrep -f` 자기 매칭 — 대기 스크립트가 끝나지 않는다

`until ! pgrep -f "nlm_episode_oneshot.py --title 홍계월전"` 은 **대기 명령 자신의 명령줄**에 그 문자열이 있어
영원히 참이 된다. 원격 `ssh '... pgrep -f 10_create_video ...'` 도 같다(최척전 때 「렌더 실행 중」 오탐의 원인).
- ★ **패턴 첫 글자를 대괄호로 감싼다** — `pgrep -f "[1]0_create_video"`. 정규식은 같은 걸 찾지만 명령줄 문자열과는 다르다

---

## 2026-09-24 — 고전소설 후보 2차 재공급

47건 스크리닝 → 제안어 40 → 수요 ≥300 **21** → 경쟁 <5,000 **11**. 상세 표는 `data/candidates.md` 상단.
- ★ **유명작은 전부 경쟁에 막혔다**: 구운몽 81,982 · 홍길동전 53,862 · 심청전 46,706 · 한중록 38,094 ·
  춘향전 28,118 · 흥부전 19,427. **전우치전은 3,615→21,366 진동**(영화 「전우치」)
- ★ 안정 통과 상위: 만복사저포기 305 · 정수정전 275 · 토끼전 228(⚠️채널에 3작품 묶음 일당백편 있음)
- ⚠️ 수치는 크지만 못 믿는 3건: **옥루몽**(동명 빙수 카페 체인) · **창선감의록**(네이버 2,450 vs 유튜브 제안어 1개) ·
  **두껍전**(제안어 절반이 인명)
- ⛔ 하위 검색어로 쪼개 재는 사평역 절차가 이번엔 판별력이 없었다 — 하위어가 전부 10~60회라
  머리어 합계를 설명하지 못한다(네이버가 머리어에 몰아 센다)
- ⏸ **사씨남정기 재측정 2회: 4,773 · 4,603.** 누적 6회 중 1회(9,720)만 초과 — 원미동 사람들과 같은 모양
- ★ **교과서 신호 ≠ 수요**: 예덕선생전(비상·내신·신사고) 170회 · 조침문(지학사·해냄) 260회 ·
  공방전(미래엔·비상)은 「베를린 공방전」 동음이의로 경쟁 35,143

---

## 2026-09-23 — 「최척전」(조위한) 제작·업로드, 고전소설 축 4편째

`aaNcU303Vpc`, **12/12(토) 18:00 KST 예약**. 9/21 재공급 3순위(받을 수 있는 수요 440).
채널 494편 순회로 중복 없음. 운영전·박씨전·옹고집전에 이어 **고전소설 축 네 번째**.

- 영문 제목 **The Tale of Choe Cheok** — en 위키 표제가 채널 관례(`The Tale of ○○`)와 일치했다.
  저자 매핑 `조위한 → Jo Wi-han` 신규 추가
- Summary 2,240자 → TTS **4:59** · NLM **6:19** · 렌더 **11:21 / 404MB**
  (20,434프레임, 57분, md5 `aa0b3881` 대조, 디코드 오류 0건)
- 이미지 90장 · 태그 26개 · 썸네일 검사 통과(테두리 50%) · 재생목록 추가(position 148)

### ★★ ko 위키에 문서가 **아예 없는** 첫 사례

운영전·박씨전은 stub 이라도 있었는데 「최척전」은 `list=search` 결과가 **0건**이다.
→ **en 위키를 보조 주력으로 끌어올려** 3개를 채웠다(작품 중심 서술이라 대체가 됐다).
- 소스: encykorea 작품 `E0057689`(줄거리 완결·이본·**피로 문학** 규정) +
  en 위키 `Choe Cheok jeon` + encykorea 저자 `E0052465`
- ★ **저자 소스를 처음으로 넣었다.** 조위한은 작품과 같은 전란을 직접 겪은 사람이라
  작가론 드리프트가 아니라 작품 이해로 이어진다. 다만 관직 나열 중심이라 **보조로만** 뒀다
- ⛔ **「임진왜란」·「정유재란」·「심하 전투」는 넣지 않았다** — 배경 금지 규칙(박씨전 9/21).
  ⛔ 「어우야담」도 제외 — **야담집 개론**으로 흐른다(장르 항목 금지와 같은 이유)
- ⛔ 위키문헌 불가 — **한문소설**이다. 옛한글 문제(운영전·박씨전) 이전 단계에서 걸린다

### ⚠️ Flux 서버 150:9002 가 고장나 있다 — 9010 으로 우회
...
```
- `/health` 는 `{"status":"ok","model":"FLUX.1-schnell","loaded":true}` 로 **정상이라고 답한다.**
  ★ **health 통과가 생성 가능을 뜻하지 않는다** — 실제 `/generate` 를 한 번 때려 봐야 안다
- 8/19 부터 떠 있던 프로세스(`/tmp/flux_server_9002.py`)다. **재시작하지 않았다**
  (「동작 중인 것은 건드리지 않는다」 — 대체 경로가 있었다)
- ★ **150:9010 에 정상 인스턴스가 있다**(`scripts/flux_server_9010.py`, `CUDA_VISIBLE_DEVICES=1`).
  `FLUX_SERVER_URL` 환경변수로 갈아탈 수 있다(코드 기본값은 9001 하드코딩이라 그대로는 404)

### 썸네일 — 4회 생성, 그리고 「배경만 다시 받아 오버레이」

9010 인스턴스는 **플랫 벡터 일러스트로 수렴하는 경향**이 있다. 1·2·3차 모두 검은 외곽선이
남았고, 3차까지는 피리를 어깨에 걸친 자세(노 젓는 것처럼 보임)로 나왔다.
- 해결: **화풍을 화가 이름으로 지정**(`in the style of Jeong Seon and Kim Hongdo`) +
  `NO hard black outlines, NO flat fills` + **악기 자세를 명시**
  (`holding a slender vertical bamboo flute upright against his face and blowing into its top end`
  — 퉁소는 **세로**로 부는 악기다)
- ★★ **낙관 제거는 오버레이 전 배경에서 해야 한다.** 완성본에서 지우려다 두 번 망쳤다:
  세로 보간의 아래 경계가 **「핵심요약」 노란 글자를 읽어** 노란 줄무늬가 위로 번졌다.
  → **같은 seed 로 Flux 를 다시 호출해 배경만 받고**, 글자가 없는 상태에서 지운 뒤
  `overlay_text()` 를 직접 불렀다. 생성 함수가 seed 를 받으므로 재현이 공짜다
- ⚠️ 낙관이 예상 영역보다 **위로 걸쳐** 있었다. 1차 시도에서 위 경계가 낙관에 물려 잔상이 남았다

### ⚠️ 이미지 — 140장 중 50장을 걷어냈다 (삭제율 36%)

바다·항해가 주소재라 **현대 요트·크루즈·서양 범선**이 대량으로 들어왔다.
- ★ **`old wooden sailing ship` 은 현대 요트와 서양 스쿠너를 부른다.** 성조기 달린 범선,
  크롬 윈치 클로즈업, 초호화 요트가 통과했다. 15장 중 8장 폐기
- ★ **`rice terrace`·`countryside hills` 는 중국·동남아 계단식 논을 부른다.**
  중국 롱셩의 야오족(분홍 옷·스마트폰)까지 들어왔다. **한국에 계단식 논은 드물다** —
  `korean` 을 붙여도 안 막힌다
- 그 밖에 태국 롱테일 보트, 터키풍 돔 건물, 알프스 설산, 유럽 침엽수림, 후지산,
  **두부 요리 사진**(독 짓는 늙은이에서도 나왔다), 벡터 클립아트 양반 2장을 제거
- ★ **확대 검수에서 2장을 더 잡았다** — 축소 몽타주에서 「운해」로 보이던 사진 하단에
  **야자수**가 있었다. 밝기 상위 컷 확대 검수는 글자만이 아니라 **지역 판별에도 듣는다**

### 다음 편 재료가 바닥났다

9/21 재공급 8건 중 7건을 썼다. **남은 것은 홍계월전(440) 하나뿐**이다.
다음 제작 전에 **고전소설 축 재공급**이 선행되어야 한다.

---

## 2026-09-22 — 「옹고집전」 제작·업로드, 고전소설 축 3편째

`HT8LTkdY0Xg`, **12/09(수) 18:00 KST 예약**. 재공급 2순위. 채널 493편 순회로 중복 없음.
운영전·박씨전에 이어 **고전소설 축 세 번째**.

- 영문 제목 **The Tale of Ong Gojip** — 표준 영역본이 없어 **채널 관례(`The Tale of ○○`)를 적용**했다
  (운영전·박씨전과 같은 형태). 학술 문헌은 로마자 `Onggojipjeon` 을 쓴다
- Summary 2,147자 → TTS **4:47** · NLM 7:34 · 렌더 **12:24 / 420MB**(md5 대조, 오류 0건)
- 이미지 71장 · 태그 26개 · 썸네일 검사 통과 · 재생목록 추가

### ★ ko 위키가 토막글이 아닌 드문 경우

고전소설은 대개 ko 위키가 stub 인데(운영전·박씨전 모두), **「옹고집전」은 이본 목록과
줄거리가 완결**돼 있어 그대로 주력급으로 썼다. encykorea `E0038895`(진가쟁주 유형 분석·
놀부와 함께 악인 계보) + 한국민속대백과 「옹고집타령」(판소리 전승)을 더해 3개를 채웠다.

⛔ **「판소리계 소설」(`E0070371`) 같은 장르 항목은 넣지 않는다** — 춘향전·심청전·흥부전이
함께 나열돼 **장르 개론으로 흘러간다.** 배경 금지 규칙의 **장르판**이다.

### ⚠️ 이미지 — 「한국 밖 아시아」가 계속 새어 들어온다

직전 편(박씨전)에서 「traditional·lantern 이 현대 축제를 부른다」를 배워 키워드를 바꿨는데도,
이번에는 **동남아 모내기 3장 · 중국 천단 · 일본어 습자 교본**이 들어왔다. 31장을 제거했다.
- ★ **`rice paddy`·`monk`·`brush paper`·`temple` 은 한국 밖 아시아를 끌어온다.**
  동아시아 공통 소재는 `korean`·`hanok`·`joseon` 을 붙여도 완전히 막히지 않는다.
  **공예·농경·불교 소재는 삭제율을 30% 이상으로 잡고 시작할 것**
- 그 밖에 한글 간판 상점·아파트 단지·카페 간판·현대 쌀포대(인쇄 글자)를 제거

---

## 2026-09-21 (4) — 「박씨전」 제작·업로드, 재공급 1순위

`0Hx7sXbE15A`, **12/06(일) 18:00 KST 예약**. 9/21 재공급의 1순위(받을 수 있는 수요 644).
채널 492편 순회로 중복 없음. **고전소설 축 두 번째 편**(운영전에 이어).

- 영문 제목 **The Tale of Lady Pak**(en 위키 `Pak ssi jeon`)
- Summary 2,190자 → TTS **4:53** · NLM 8:05 · 렌더 **13:00 / 422MB**(md5 대조, 오류 0건)
- 이미지 76장 · 태그 26개 · 썸네일 검사 통과 · 재생목록 추가
- 작자 미상이라 저자 소스가 없다(운영전과 같음) →
  encykorea `E0020897`(이본 3유형·전후반 이원 구조) + **문화포털 예술지식백과**(줄거리 800자+·
  변신 모티프·여걸소설 분석) + ko 위키(stub)
  ★ **문화포털 예술지식백과가 새 소스원으로 쓸 만하다** — 작자 미상 고전에 encykorea 만으로
  분량이 모자랄 때 대안이 된다
- ⛔ 「병자호란」·「군담소설」 항목은 넣지 않았다. **배경이 실제 전쟁인 작품은 역사 강의로
  흘러갈 위험이 다른 편보다 크다**
- ⛔ 위키문헌에 한글 전문이 있으나 **옛한글**이라 제외(운영전과 동일 판단).
  ★ 고전소설은 저작권이 만료됐어도 **옛한글 문제로 위키문헌을 쓸 수 없는 경우가 많다**

### 이미지 — 연등 축제 사진 7장이 섞였다

`korean traditional lantern night` 키워드가 **현대 연등회(불교 행사)** 사진을 끌어왔다.
작품과 무관하고 색감도 튄다. 그 밖에 김치 요리·한복 체험 관광객·한옥 뒤 아파트 등
합계 24장을 육안으로 제거했다.
- ★ **「traditional」·「lantern」 같은 말은 현대 축제 사진을 부른다.** 전통 소재 키워드에는
  행사·축제로 소비되는 것(연등·탈춤·한복체험)이 섞이니 결과를 반드시 눈으로 볼 것

### 썸네일 — 인물의 성별이 읽히는지까지 본다

3회 생성 중 1·2번은 인물이 **도포 차림 남성처럼** 보였다. 이 작품은 여성 영웅소설이라
그대로 쓰면 핵심이 어긋난다. 프롬프트에 `wide billowing hanbok skirt and short jeogori jacket`
처럼 **복식을 명시**해서 3번에서 해결했다. 좌하단 낙관은 세로 보간으로 제거.

---

## 2026-09-21 (3) — 후보 재공급: 고전소설 축이 근현대 축을 이겼다

풀이 바닥나 45건을 새로 스크리닝했다. **두 축의 결과가 정반대로 갈렸다.**

| 축 | 스크리닝 | 수요 통과 | 최종 통과 |
|---|---:|---:|---:|
| 근현대 교과서 단편 | 30건 | 7건 | **2건** |
| **고전소설** | 15건 | 8건 | **8건**(즉사 0) |

### ⛔ 근현대 축 — 수요가 큰 작품은 경쟁도 크다
...
★ **「교과서 수록 + 유명」은 이미 남들도 만들었다는 뜻이다.**
수난이대(5,190 / 747)나 원미동 사람들(4,820 / 2,411) 같은 조합이 희귀한 이유가 여기 있다.
발굴 공식에 **「유명하지 않을 것」**이 암묵적으로 들어 있었다는 점이 이번에 드러났다.

동음이의 탈락: 낙동강=강 이름·녹조 / 물레방아=나훈아 노래 / 서편제=영화 OST /
나목=「나모긴디」 오타 검색어 / 어둠의 혼=롤 스킨 / 빈처=가요 / 두 파산=성인물.
제안어 0(측정불가): 해산 바가지·산골 나그네·광염 소나타·나무들 비탈에 서다.

### ✅ 고전소설 축 — 15건 중 하나도 즉사하지 않았다

제목이 전부 **「○○전」 형태의 고유명사**라 동음이의가 구조적으로 없다.
교과서 신호(비상·신사고·창비·수특·내신·중3)도 줄줄이 붙는다. **운영전과 같은 축이다.**

새 1순위는 **박씨전**(수요 1,400 / 경쟁 2,745~2,902, 의도 0.46 → 받을 수 있는 수요 **644**).
2위 옹고집전 528 · 3위 최척전 440 · 4위 홍계월전 440.

⏸ **사씨남정기 보류** — 수요 2,490에 의도 0.38이라 받을 수 있는 수요 946 으로 최상위급인데,
경쟁 4회 중 **1회가 게이트를 94% 초과**(9,720, 나머지 3회는 3,064). 침묵의 봄과 같은 패턴이라
**빈도(1/4)만이 아니라 폭(94%)을 함께 보고** 보류로 내렸다. 재측정 가치는 있다.

★ **다음 재공급은 고전소설을 먼저 판다**: 유충렬전·장끼전·토끼전·옥루몽·심청전 계열.

---

## 2026-09-21 (2) — 「독 짓는 늙은이」 제작·업로드

`uA7G9R_f6vA`, **12/03(목) 18:00 KST 예약**. 후보 3순위. 채널 491편 순회로 중복 없음
(걸린 것은 여러 작가 **선집 소개편** 「한국의 단편」이라 단독 핵심요약과 다르다).

- 영문 제목 **The Old Jar Craftsman**(en 위키 표제). 저자 매핑도 신규 추가
- Summary 2,178자 → TTS **4:49** · NLM **9:49**(평소보다 김) · 렌더 **14:41 / 525MB**
  (26,447프레임, 약 1시간 10분, md5 대조, 오류 0건)
- 이미지 81장 · 태그 26개 · 썸네일 검사 통과(테두리 75%) · 재생목록 추가
- NLM 소스: 1950년작이라 위키문헌 불가(저작권 유효) →
  encykorea 작품 `E0016038` + encykorea 저자 `E0071297` + ko 위키(stub)

### ★★ 공예·기술 소재는 AI 검증이 지역을 못 거른다 — 누적 57장 폐기

「도자기」가 전 세계 공통 소재라, 검증을 통과한 100장 안에
**모로코 가죽 염색장(Fez) · 이탈리아 마욜리카 화병 · 현대 도예 체험 교실 ·
서양 목공방 · 두부와 샤브샤브 요리 사진**까지 들어와 있었다.
3라운드(210 → 150장 보충 → 재검증) 동안 **57장을 육안으로 걷어냈다.**

- ★ **제주 풍경·한옥처럼 지역이 한정된 소재와 다르다.** 「독을 짓는다」는 행위 자체는
  어느 문화에나 있어서 점수로는 구분되지 않는다. **공예·기술·직업 소재의 책은
  키워드에 `korean`·`onggi`·`hanok` 같은 지역 한정어를 반드시 붙이고,
  그러고도 육안 검수에서 절반 가까이 빠질 것을 예산에 넣는다**
- 1차 삭제 후 62장이 남아 **필요분(64장)에 미달**했다. 보충 라운드를 한 번 더 돌려 81장 확보.
  ⚠️ 삭제량이 클 수 있으니 **필요분 계산(TTS초 ÷ 4.5)을 삭제 전에 해 둘 것**

### 썸네일 — 낙관 제거는 「열별 세로 보간」이 답이다

4회 생성 모두 가짜 낙관·서명이 붙었다(한 장은 낙관 3개, 한 장은 깨진 한글 서명).
9/19 에 쓴 「같은 대역 복사」가 **패널 경계에 걸리면 옅은 사각형이 남는다**(2회 실패).
- ★ **해법: 지울 영역의 위·아래 경계 색을 열마다 읽어 세로로 선형 보간하고 미세 노이즈를 얹는다.**
  가로 그라데이션이 그대로 보존돼 자국이 남지 않는다. 우상단 세로 한자를 이 방법으로 지웠다
- 단순한 평면 배경(좌하단 낙관)에서는 기존 「같은 x 대역 복사」로 충분했다

---

## 2026-09-21

### 「양반전」(박지원) 제작·업로드 — 후보 2순위

`iyR04wtaFYE`, **11/30(월) 18:00 KST 예약**. 중복 확인은 채널 490편 전량 순회
(「열하일기」가 걸리지만 **같은 저자 다른 작품**이라 중복 아님).

- 영문 제목 **The Tale of a Yangban**(LTI Korea·Britannica). 저자 매핑은 기존에 있어 제목만 추가
- Summary 2,235자 → TTS **4:58** · NLM 6:52 · 렌더 11:52.7 / 430MB(GPU150, md5 대조, 오류 0건)
- 이미지 75장(저점수 0, 어두운 컷 최대 연속 1장. 필요분 66장)
- 태그 26개(방경각외전·정선 양반·실학 + 교과서 축) · 설명은 **두 증서 구조**를 축으로 작성
- ★ 썸네일이 **수정된 네이밍 규약대로 저장돼 업로드 검사를 그대로 통과**했다(테두리 75%).
  9/19 사고 대응이 바로 다음 편에서 효과를 확인했다

### ⚠️⚠️ encykorea 에 「양반전」이 세 개 있다 — 번호를 반드시 확인할 것

| ID | 항목 | 판정 |
|---|---|---|
| `E0035522` | **兩班傳** — 박지원의 한문소설 | ★ 이것을 쓴다 |
| `E0035523` | 兩班田 — **고려 전시과의 토지제도** | ⛔ 넣으면 토지제도 강의가 된다 |
| `E0035521` | 양반 — 신분 계층 | ⛔ |

**검색에서 토지제도(`E0035523`)가 먼저 나왔다.** 한글 표기가 같아 육안으로 구분되지 않는다.
소스 URL 파일에 경고로 남겼다. **동음이의 항목이 있는 고전은 한자까지 확인하고 넣는다.**

⛔ 위키문헌 『연암집』 제8권 별집은 **한문 원문**이라 제외(운영전과 동일 판단).
→ encykorea 작품 + LTI Korea 저자(2,500자) + ko 위키(stub, 용어용) 조합.

### 썸네일 — Flux 가 매번 가짜 낙관을 찍는다

3회 생성 모두 서명·낙관이 붙었다(`Ⓡ Annle`, 붉은 도장). **재생성으로 없어지지 않는다**는
기존 관찰이 재확인됐다. 이번에는 낙관이 **우상단**이라 「하단 10% 크롭」 요령이 통하지 않았다.
- ★ **해법: 같은 이미지 안에서 좌우 밝기 그라데이션이 동일한 대역(같은 x, y만 이동)을 떠다 덮는다.**
  중앙값 단색으로 채우면 벽의 그라데이션과 어긋나 옅은 사각형이 남는다(2회 실패 후 성공)

### ★ `pgrep -f` 가 ssh 명령 문자열 자신에 매칭된다 — 이 세션에서 네 번 걸렸다

렌더 완료 감시가 30분씩 두 번 헛돌았다. `ssh <host> "pgrep -f 'src/10_create_video...'"` 는
**그 ssh 가 띄운 원격 bash 의 명령줄에 패턴이 들어 있어** 자기 자신을 세고 영원히 「실행 중」이 된다.
Xvfb 생존 확인, 렌더 중복 확인에서도 같은 오판이 났다.
- ★ **확인은 `pgrep -af '<실행 바이너리 경로>' | grep -v 'bash -c'`** 처럼 자기 매칭을 배제한다
  (예: `render_venv/bin/python`). X 서버는 프로세스가 아니라 **`xdpyinfo` 응답**으로 판정한다

---

## 2026-09-19

### 「순이 삼촌」(현기영) 제작·업로드 — 후보 1순위

`AU6rHiUO1gk`, **11/27(금) 18:00 KST 예약**. 중복 확인은 채널 490편 전량 순회로 했다(없음).

- 영문 제목 **Aunt Suni**(2010년 실제 영역본). 재번역본 `Sun-i Samch'on` 은 어포스트로피가
  파일명에 부적합해 쓰지 않았다. `translations.py` 에 제목·저자 매핑 추가
- Summary 2,254자 → TTS **5:02**(예상 5:03, 오차 1초). voice `nova` 고정
- NLM 6:45 · 렌더 11:49.8 / 446MB(GPU150) · md5 대조 후 회수 · 무결성 오류 0건
- 태그 26개(교과서 축 포함) · 「📖 책 소개」·영문 블록 제거 · 재생목록 추가

★ **NLM 소스 3개 중 위키를 쓸 수 없었다.** 작품·저자 ko 위키가 둘 다 stub 인데,
1978년작이라 **「stub 이면 위키문헌 전문으로 대체」 규칙이 통하지 않는다**(저작권 유효).
근대 단편(B사감 등)과 다른 경우다. **한국민족문화대백과 + LTI Korea(약 8,000자)** 로
분량을 확보했다. 앞으로 **생존 작가·현대 작품은 이 조합을 먼저 본다**.

### ★★ 썸네일 네이밍 불일치 — 배경 그림이 올라갈 뻔했다

업로드 직후 검사 장치가 **「mood_001.jpg · 템플릿 테두리 0%」**를 띄웠다.
9/11 에 붙여둔 그 검사가 처음으로 실제 사고를 잡았다.

- **원인**: `thumbnail_generator.py` 는 **한글 제목**으로 저장하고
  (`output/순이_삼촌_thumbnail_ko.jpg`), `09_upload_from_metadata.py` 는
  **영상 stem 기준**(`output/Aunt_Suni_kr_thumbnail_ko.jpg`)만 찾는다.
  못 찾자 `input/` 폴백이 돌아 **무드 이미지 한 장이 썸네일로 올라갔다**
- **조치**: 생성기가 `translate_book_title()` 로 영문 제목을 얻어
  `{영문제목}_{kr|en}_thumbnail_{ko|en}.jpg` 로 저장하도록 고쳤다.
  기존 파일(`Two_Generations_of_Suffering_kr_thumbnail_ko.jpg`)과 같은 규약이다
- ⚠️ **이번 편 한정이 아니라 두 스크립트의 규약 불일치였다** — 고치지 않았으면 다음 편도 같다
- 교체 후 **실제로 내려받아 눈으로 확인**했다. API 의 `maxres` 존재만으로는 판단하지 않는다

### Flux 서버 — 9002 가 고장나 있다

- `150:9002` 는 `/health` 가 정상(`loaded: true`)인데 `/generate` 는 500 을 낸다:
  `Expected all tensors to be on the same device, but found at least two devices, cpu and cuda:0`
- **`150:9010`**(8/30부터 상주)이 정상이라 그쪽으로 생성했다. 9002 는 손대지 않았다
- ⚠️ `/health` 200 을 기동 확인으로 쓰면 안 된다. **실제 `/generate` 1회**가 판정 기준이다

### 이미지 — AI 검증 통과분에서 33장을 육안으로 걷어냈다

3라운드(197 → 160 → 217장 수집)를 거쳐 **저점수 0개 / 89장** 확보.
검증을 통과하고도 부적합했던 것: 동남아 계단식 논·물소 농부, 붉은 용암 분출,
열대 야자수 해변, 유럽 벽돌집·폐공장, 현대 관광지(벤치·핑크뮬리), 한옥 뒤 고층아파트,
**한글이 찍힌 아이스크림 컵**, 동남아·남아시아 노인 인물 4장.
밝기 교차 배치 후 **어두운 컷 최대 연속 1장**.

---

## 2026-09-18

### 남은 후보 3건 재측정 — 순위를 「받을 수 있는 수요」로 다시 매겼다

착수 대기 중이라 제작은 하지 않되, 다음 착수 때 필요한 값을 미리 갱신했다.
수요 2회 · 공급 4회 측정.

| 책 | 수요 (전→후) | 경쟁 (전→후) | 제안어 의도 | 받을 수 있는 수요 |
|---|---|---|---:|---:|
| 순이 삼촌 | 1,370 → **1,350** | 2,726~3,316 → 1,947~3,319 | 2/2 | **≈1,350** |
| 모래톱 이야기 | 370 → **350** | 969~1,006 → 974~1,046 | 1/1 | ≈350 |
| 이중나선 | 930 → **870** | **752 → 266~857** | 3/7 | ≈374 |

- ★ **이중나선을 1순위에서 4순위로 내렸다.** 게이트는 최대값 857 로 통과하지만,
  수요가 870 뿐이고 제안어 7개 중 3개만 책 의도라 실제로 받을 수 있는 수요가 약 374 다.
  「비율이 아니라 받을 수 있는 수요로 순위를 매긴다」(9/12) 원칙을 그대로 적용했다.
  경쟁도 266~857 로 3배 진동해 752 단일값은 재현되지 않았다
- **순이 삼촌이 1순위**가 됐다. 수요 1,350 에 제안어가 전부 책 의도다
### ★ 스크리닝은 변형 표기와 저자명을 함께 넣어야 한다

위 재측정에서 순이 삼촌·독 짓는 늙은이·모래톱에 「제안어가 1~2개뿐이라 의도 1.00 의
신뢰도가 낮다」는 경고를 달았는데, **띄어쓰기 표기 하나만 본 탓이었다.** 셋 중 둘은 오해였다.

| 책 | 변형 합산 제안어 | 판정 |
|---|---|---|
| 순이 삼촌 | **8개** (`해석·오디오북·오페라·`**`내신·수특`**) | ⚠️ **철회** — 교과서 신호 확인 |
| 독 짓는 늙은이 | **6개** (`줄거리 요약·줄거리·오디오북·해설`×2·`황순원 독짓는 늙은이`) | ⚠️ **철회** — 의도 깨끗 |
| 모래톱 이야기 | **1개** (`줄거리`, 두 표기 모두 동일) | ⚠️ **유지** — 검색 깊이가 얕다 |

- ★ **절차로 굳힌다: `screen_candidates.py` 는 변형을 자동 생성하지 않는다.**
  띄어쓰기 유무 전부(`순이 삼촌`/`순이삼촌`, `독 짓는 늙은이`/`독짓는 늙은이`/`독짓는늙은이`)와
  **저자명**을 함께 인자로 넣어야 실제 그림이 보인다
- ★ **저자명 스크리닝은 「보조 유입 경로가 있는가」를 본다.** 우리 제목이 저자 제안어에
  뜨면 그 경로로도 들어온다. **황순원은 소나기가 13개를 압도**해 독 짓는 늙은이가 없고,
  **김정한은 제안 13개 중 책 의도가 0**(한국사 강사·목사·주식이 점유)이다. 둘 다 저자 경로가 막혔다
- 모래톱 이야기는 **수요 하한(350 vs 게이트 300) + 검색 깊이 얕음 + 저자 경로 막힘**이
  겹친다. 게이트는 통과하지만 5순위가 타당하다
- 세 건 모두 게이트(수요 ≥ 300 / 경쟁 < 5,000) 통과. 모래톱 이야기는 수요 350 으로 하한 근처

---

## 2026-09-16

### 「양반전」 재측정 — 수요는 안정, 경쟁은 진동

착수 전 재측정 규칙에 따라 다시 쟀다.

| 축 | 9/13 | 9/16 재측정 | 판정 |
|---|---|---|---|
| 수요 | 1,680 | **1,730** (2회 동일) | 안정 (+3%) |
| 경쟁 | 3,994 (3회 동일) | 3,287 / 4,000 / 3,287 / **2,097** | **진동 2,097~4,000** |

- 게이트는 **관측 최대값 4,000** 으로 판정 → 5,000 미만이라 통과. 비율 0.43~0.83
- 제안어는 **해석·중2·박지원·오디오북·애니메이션·분석** 로 교과서 축이 뚜렷하다.
  비책 의도는 「양반전복죽」 하나뿐 (책 의도 비율 0.31, 「경쟁낮음」)
- ★ **연속 동일값은 안정의 근거로 약하다 — 단 반례는 양반전 하나였다.**
  9/13 에 3회 연속 3,994 로 나와 안정 그룹으로 적어뒀는데, 사흘 뒤 4회 측정에서
  한 번도 3,994 가 나오지 않았다.
  ⚠️ **처음엔 "연속 동일값은 근거가 못 된다"고 일반화했으나 9/16~18 재측정에서 완화했다** —
  운영전(2,134→2,139~2,320)·독 짓는 늙은이(1,305→1,306~1,423)는 다른 날짜에 오차 0.2~9% 로
  재현됐다. 연속 동일값도 대체로 재현되지만 **양반전 같은 반례가 있으므로
  착수 직전 재측정은 여전히 해야 한다**
- `data/candidates.md` 3순위 행을 범위 표기로 갱신

---

## 2026-09-11

### ★ 고정 댓글 실험 판독 — 효과 없음 (8/27 고정 → 15일)

9/10 전후로 잡아둔 체크포인트를 지나쳐 있었다. `DISPLAY=:99 python3 scripts/check_pinned.py` 로 판독했다.

| | 좋아요 변화 | 답글 변화 |
|---|---|---|
| 고정 5편 (히치하이커·생각에 관한 생각·나는 고양이로소이다·특이점·모비딕) | **0→1** (히치하이커 1건뿐) | 전부 0→0 |
| 미고정 4편 (금병매 한/영·데미안·행동하지 않으면) | 전부 0→0 | 전부 0→0 |

- **15일간 차이가 좋아요 1건.** 고정의 참여 유발 효과는 사실상 없다
- → **수동 고정 절차를 중단한다.** API 가 고정을 지원하지 않아 매번 Studio 에서 손으로
  해야 했는데, 그 수고를 들일 근거가 없다. Playwright 자동화도 만들 필요 없다
- 댓글 자체는 제휴 링크가 있으므로 16:10 cron 으로 계속 단다

### ★ 업로드 스크립트에 썸네일 텍스트 검사를 연결했다

9/08 사고(훅·제목·저자 없는 배경 그림 8편이 공개됨)의 재발을 구조적으로 막는다.

- `09_upload_from_metadata.py` 의 `upload_thumbnail()` 이 **업로드 직전에 로컬 파일을 검사**한다.
  올라간 뒤 확인하면 이미 늦다
- 이상은 `thumbnail_warnings` 에 모아 **마지막 요약에 Studio 편집 링크와 함께** 출력한다
  (중간 로그에 묻히면 못 본다)
- 판정은 현행 템플릿(노란 테두리) 기준이라 **차단하지 않고 경고만** 한다
- 검증: 만세전 원본(배경만) 0% → ❌ / 수정본 70% · B사감 75% → ✅ / 파일 없음 → 통과 처리

### ⛔ 채널 전수 썸네일 점검은 성립하지 않는다 (두 번 오탐)

9/08 누락이 다른 편에도 있는지 460편을 검사했다가 **두 번 연속 오탐**을 냈다.

1. **노란 테두리 기준** → Gems 시대의 **파란 프레임** 35편이 전부 "누락"으로 잡혔다
   (살인자의 기억법·일리아스·파친코 등 — 몽타주로 확인하니 전부 정상)
2. **'상단의 흰 글씨' 기준**(엣지 밀도 + 획 검출)으로 교체 → 로컬 검증은 통과했으나
   (없음 0.16 / 있음 0.37~1.00) 전수에서 또 7편이 걸렸다. 종의 기원(빨강·노랑 글씨)·
   목민심서·금병매·이반 일리치·굿모닝 미드나잇 — 색이 흰색이 아니거나 텍스트가
   중앙·하단에 있는 경우다. **역시 전부 정상이었다**

→ 채널의 썸네일 스타일이 시기마다 달라 자동 전수 판정은 안 된다.
**우리 파이프라인이 만든 것만** 판정 가능하고, 그건 노란 테두리로 충분하다.
★ 실질 결론: **9/08 누락은 새 기준 8편에만 있던 문제였고 이미 고쳤다. 나머지 452편은 정상.**

### 우상의 눈물 공개 확인 (9/10 18:00)

`verify_published.py` 로 점검 — 공개 전환·썸네일 텍스트·태그 26개·재생목록 전 항목 통과.
16:10 cron 은 공개 전이라 댓글을 못 달았고, 9/11 에 수동으로 추가했다
(`UgyLlPfb3_TodBffvFV4AaABAg`). 공개 17시간 시점 조회 5회 — 만세전과 같은 곡선이다.

---

## 2026-09-09

### ★★ 네이버 검색광고 API 연결 — 수요 축이 실제로 후보를 걸러냈다

어제 붙인 수요 지표에 네이버 키를 연결했다. **월간 절대 검색수**를 최소 10회까지 준다.
Google Trends 의 두 한계(0 반올림 · 20~30건이면 429)가 모두 해소됐다.

- ⚠️ **네이버 광고 UI 가 개편됐다.** `ads.naver.com` **광고주센터**로 바뀌었고,
  API 메뉴는 **좌측 사이드바에서 「검색 광고」로 전환한 뒤 「도구 > API 사용 관리」**다.
  예전 `searchad.naver.com` 의 상단 메뉴바가 아니다. 기본값이 「디스플레이 광고」로
  잡혀 있으면 도구 메뉴에 API 항목이 없다
- 광고 집행·비즈머니 충전은 **필요 없다**. 잔액 0원에 등록 광고 0건이어도 조회된다

### ★ 지표별 상관 — 수요 단독이 가장 낫고, 비율은 최악이다

같은 성숙 표본(n=20)에서 세 가지를 비교했다.
...
- ⛔ **비율만으로 순위를 매기면 안 된다.** 경쟁 0인 책(까라마조프·세계의 단편·파이프)이
  비율 최상위로 올라가는데 실제로는 무검색이다. → **게이트를 먼저 통과시킨 뒤** 비율을 쓴다
- ⚠️ 수요 단독 ρ=+0.338 도 **n=20 에서 유의하지 않다(p≈0.15)**. 방향이 맞다는 것이지
  확증이 아니다. 12월 판독 때 다시 검증한다
- 수요만으로도 안 맞는 사례: **프로젝트 헤일메리 수요 123,000회/월인데 조회 81회** —
  경쟁이 281,959로 압도적이라 우리가 상위에 못 걸린다. 두 축이 다 필요한 이유다

### ★ 판정 규칙: 수요 ≥ 300회/월 AND 경쟁 < 5,000, 그 안에서 비율 순위

**수요 축 도입으로 후보 3건이 새로 탈락했다.** 전부 공급만 볼 때는
"경쟁이 낮아 좋은 후보"였던 것들이다 — 「파이프 이야기」와 같은 함정이었다.

| 책 | 수요/월 | 경쟁 | 기존 판정 |
|---|---:|---:|---|
| 논 이야기 (채만식) | **140** | 1,037 | 통과였음 |
| 타인의 방 (최인호) | **140** | 1,453 | 통과였음 |
| 전체주의의 기원 (아렌트) | **120** | 1,330 | 하한 밖 → 확정 탈락 |

### ★★ 새 선정 기준이 옳았다는 첫 정량 증거

옛 기준 편 중 조회 1위였던 「특이점이 온다」의 자리가 **수요 2,030 / 경쟁 3,072 = 0.66** 이다.
**새 기준 9편 중 5편이 그보다 좋은 자리**에 있다.

| 책 | 수요/월 | 경쟁 | 비율 |
|---|---:|---:|---:|
| **수난이대** | **5,190** | 747 | **6.95** |
| 만무방 | 960 | 499 | 1.92 |
| 니코마코스 윤리학 | 1,530 | 872 | 1.75 |
| 기억 속의 들꽃 | 1,630 | 1,496 | 1.09 |
| B사감과 러브레터 | 800 | 851 | 0.94 |
| *(비교) 특이점이 온다* | *2,030* | *3,072* | *0.66* |

**수난이대는 계량한 것 중 압도적으로 좋다**(9/14 공개 예정) — 결과가 곧 나온다.

### 후보 풀 재정렬 — 다음 편 1순위가 바뀌었다

전 후보를 수요×공급으로 다시 계량해 `data/candidates.md` 표를 새로 세웠다.

- **1순위 원미동 사람들**(양귀자, 4,810/2,099 = **2.29**) · 2순위 당신들의 천국(1,630/817 = 2.00)
- 수요 최상위는 **꺼삐딴 리 7,320** · **침묵의 봄 7,160** 이나 둘 다 경쟁이 5,000 근접이다
- ⛔ 다만 **미리 쌓지 않는다** — 9/10 우상의 눈물 공개 반응을 보고 착수를 판단한다
...
### ★ 「원미동 사람들」(양귀자) 제작·업로드 — 새 선정 기준 10번째 편

`IAGPFO7WxoU` · **2026-11-15(일) 18:00 KST 예약**(큐 꼬리 11/12 다음 슬롯, 밀기 없음) ·
13분 41초 / 1920×1080 30fps / 510MB · 재생목록 `[KOR] 책 요약 - 전체 모음` 139번

| 항목 | 값 |
|---|---|
| 수요 | 4,820 회/월 (9/09 4,810 → 편차 0.2%) |
| 경쟁 중간값 | 2,411 (6회 측정) |
| 비율 | **2.00** |
| TTS | OpenAI tts-1-hd **nova** 5:31 (1,834자) |
| NLM | 8:07 — 초점이 작품에 정확히 잡혔다(부천으로 밀려난 현실·은사시나무) |
| 이미지 | 82장 = 실사 42 + Flux 40 |

### ⚠️ 경쟁값이 6회 중 1회만 튀었다 — 규칙 예외로 착수했다

재측정 6회: 2,099 · 2,411 · **5,133** · 2,237 · 2,411 · 2,411.

- 기록된 규칙("게이트 판정은 관측 최대값")대로면 5,133 은 게이트(5,000)를 **2.7% 초과**해 탈락이다
- 그러나 나머지 5회가 2,099~2,411 에 몰려 있어 **이상치로 판단하고 착수**했다(사용자 결정)
- ★ **12월 판독 때 이 편은 「규칙 예외로 넣은 편」으로 구분**해서 볼 것. 규칙을 우회한 표본을
  규칙의 증거로 쓰면 안 된다
- 수요는 두 번 다 4,810~4,820 으로 **안정**했다. 경쟁만 흔들린다는 8/29 관찰과 일치한다

### 스톡 195장 중 38장이 유럽·중국이었다 — 만세전 패턴의 재현

AI 검증(195→80, 저점수 0개)을 **통과한 뒤에도** 육안 검수에서 38장을 버렸다.
유럽 자갈길·베트남 호이안·모로코 골목·북촌 한옥(시대 불일치)·제품컷(소니 카세트·앤티크
스토브)·「10 PLUS」 간판. **1980년대 한국 변두리는 무료 스톡에 없다.**

- ✅ 처방은 만세전과 같다 — 실사 42 + Flux 40 혼합
- ★ **Flux 44장 중 6장을 글자 때문에 버렸다.** 깨진 한자 간판(구멍가게 내부)·벽 낙서·
  A자 입간판·「AT」. 소재로 막았는데도 **가게·표지판을 그리면 글자가 따라온다**
  → 추가 8장은 골목·밭·옥상·빨래처럼 **글자를 부를 소재가 없는 장면**으로만 뽑았고 전부 깨끗했다
- ⚠️ **축소 시트로는 못 잡는다.** 의심 컷을 940px 로 다시 보고서야 간판 글자가 보였다

### 밝기 교차 배치를 스크립트로 만들었다 — `scripts/reorder_by_brightness.py`

지금까지 편마다 손으로 하던 것을 스크립트로 고정했다.
밝기 오름차순 → 절반씩 밝은/어두운 교차 → `mood_001..NNN` 재명명 → `image_order.json` 기록.

- 결과: 82장 밝기 중간값 128.2, **30 미만 3장의 최대 연속 1장(4.5초)**
- 니코마코스의 "20초 검은 화면"이 구조적으로 막힌다

### 렌더 완료 감지 폴링이 자기 자신을 잡았다 (30분 손해)

`pgrep -f "10_create_video_with_summary"` 가 **그 패턴을 담은 SSH 명령 자신**을 매칭해
렌더가 끝난 뒤에도 계속 RUNNING 으로 보고했다. 로그를 직접 보고서야 09:47 에 이미
완료된 것을 알았다.
→ **원격 프로세스 감시는 `pgrep` 대신 로그의 완료 문자열이나 출력 파일 mtime 으로 판정할 것.**

### ★ 「당신들의 천국」(이청준) 제작·업로드 — 같은 날 두 번째 편

`72vRukqX0Cw` · **2026-11-18(수) 18:00 KST 예약** · 15분 21초 / 1920×1080 30fps / 534MB ·
재생목록 140번 · 수요 1,170 / 경쟁 835(3회 동일, 안정 그룹) · TTS nova 5:17 · 이미지 79장(실사 51 + Flux 28)

### ⚠️ NLM 1차가 「작가론」으로 나왔다 — 저자 위키가 초점을 가져간다

작품·저자(ko)·저자(en) 3개로 돌렸더니 비디오가 **「거장 이청준의 문학 세계」**가 됐다.
목차가 「거장 이청준의 탄생 / 정치 권력과 지성 / 고향과 전통 예술 / 스크린 위에서 부활한 명작」이고
판소리·서편제·천년학·수상 내역이 큰 비중이었다. **당신들의 천국은 연표 표에 한 줄뿐이었다.**

- **원인: ko 저자 위키가 8,000~10,000자로 작품 위키보다 훨씬 풍부했다.**
  기억 속의 들꽃에서 6·25 위키가 초점을 가져간 것과 같은 구조인데, 이번엔 **배경이 아니라 저자** 쪽이다
- ★ **처방: 저자 위키가 작품 위키보다 크면 ko 저자를 빼고 작품 소스를 2개로 만든다.**
  2차는 작품 위키 + **한국민족문화대백과 작품 해설**(`encykorea.aks.ac.kr/Article/E0070479`) +
  en 저자(짧음)로 돌렸고, 「당신들의 천국: 유토피아의 환상과 현실」로 정확히 잡혔다
  (자선과 권력의 변증법 · 권력 포기/기다림/민간인 귀환 · 마무리 질문까지 작품 그대로)
- ★ **소스 3개 원칙은 "작품 1 + 저자 2"가 아니라 "작품에 무게를 싣는 3개"로 읽어야 한다**
- ⚠️ **재실행하면 `SKIP: 이미 존재`로 1차 산출물을 그대로 돌려준다.**
  `input/{제목}_Part1_video_kr.mp4` 를 먼저 치워야 재생성된다 (작가론 버전은 `.AUTHOR_DRIFT.mp4` 로 보관)

### ★ 「침묵의 봄」(레이첼 카슨) — 교훈을 착수 전에 적용한 첫 편

`lVVu_nlKd5Q` · **11/21(토) 18:00 KST 예약** · 12분 33초 · 이미지 83장(실사 75 + Flux 8) ·
재생목록 141번. **발굴 공식(한국 근현대문학 + 교과서) 밖의 첫 편**이라 12월 판독에서
공식의 경계를 확인할 표본이 된다. 제안어에 **「침묵의 봄 세특」**(생기부)이 있어 매년 반복되는 수요가 있다.

- ✅ **당신들의 천국 교훈을 착수 전에 적용했다** — 위키 분량을 먼저 비교:
  ko 저자 15,000~20,000자 > ko 작품 **stub**. 저자를 빼고 **en 작품 위키를 주력**으로 썼고,
  결과는 작품 중심으로 정확했다(세상을 깨운 책 / 화학 기업들의 반격 / DDT 금지 / 계속되는 논쟁).
  ★ **소스 선정 전에 분량 비교를 하는 것이 절차가 됐다**
- **자연·새 소재는 스톡이 충분하다** — 80장 중 5장만 폐기. 시대·지역이 특정된 작품
  (만세전·원미동·당신들의 천국)과 정반대다. **"스톡이 약하다"는 작품 성격에 따라 갈린다**
- ⚠️ **업로드 직후 API 조회에서 태그가 0으로 보였다.** 1분 뒤 재조회하니 26개 정상 —
  **전파 지연이다.** 업로드 직후 값으로 태그 소실 버그를 판단하지 말 것

### ★ 「운영전」(작자 미상) — 풀 재공급 1순위를 바로 제작 (2026-09-13)

`0eWjThwdbd4` · **11/24(화) 18:00 KST 예약** · 14분 20초 · 이미지 79장(실사 69 + Flux 10) ·
재생목록 142번 · 수요 2,110 / 경쟁 **2,134(4회 전부 동일)**.

- ★ **작자 미상이라 작가론 드리프트 위험이 구조적으로 없다** — 저자 소스가 애초에 없다.
  한국민족문화대백과 작품 해설 + ko 위키(stub) 2개로 충분했고 초점이 정확했다
- **위키문헌 전문을 일부러 뺐다** — 운영전 전문은 **옛한글 24회차**라 노이즈가 된다.
  B사감(현대 한글 전문)과 달리 **고전은 표기가 장벽**이다
- ⚠️ **썸네일 1차가 애니메이션풍으로 나왔다.** `painted illustration` 만으로는 부족하다 →
  `traditional Korean ink wash painting on mulberry paper, visible brush strokes,
  NOT anime, NOT digital cartoon` 으로 재생성해 해결(20초). **톤이 튀면 재생성이 싸다**
- 궁궐 장면은 **현판·간판을 부른다** — Flux 12장 중 2장에 글자가 생겨 폐기

### ⛔ 「사평역」은 탈락시켰다 — 수요 3,670 의 정체가 지하철역이었다

착수 직전에 검색어를 쪼개 다시 쟀더니 **「사평역 소설」이 월 55회**였다.
...
- ko 위키 「사평역」은 **서울 지하철 9호선 역** 문서다(소설 문서는 아예 없다)
- ★ **교훈: 제목이 실재 지명·역명과 겹치면 수요 지표가 통째로 오염된다.**
  유튜브 자동완성이 문학으로 보인다고 안심하지 말 것 — 네이버 절대 검색수는 다른 시장을 센다
- ★ **검증법: 「<제목> 소설」처럼 의도를 못박은 키워드를 함께 재서 비율을 본다**

### ⛔ 원격 프로세스 감시에 pgrep 을 쓰지 말 것 (하루에 두 번 당했다)

`pgrep -f "10_create_video_with_summary"` 와 `pgrep -f render_paradise` 가 **그 문자열을 담은
내 SSH 명령줄 자신을 매칭**해, 렌더가 끝났는데도 RUNNING 으로, 시작 전인데도 "이미 실행중"으로 보고했다.
원미동에서 30분을 헛기다렸다.

- ✅ **완료 판정은 로그의 완료 문자열**(`요약 포함 영상 제작 완료`)로 한다
- ✅ **실행 여부 판정은** `ps -eo cmd | grep -c "^/home/jsong/render_venv/bin/python"` 처럼
  **내 명령줄에 없는 문자열**로 한다

### 업로드 점검에서 걸러낸 것

- `localizations.ko` 에 **손질 전 설명**(영문 블록·「📖 책 소개」 포함)이 남아 있었다.
  본문만 고치고 끝내면 현지화 설명이 옛 문구로 남는다 → 본문과 동기화하는 단계를 넣었다
- 영문 메타데이터가 자동 생성되므로 ko-only 모드에서는 **업로드 전에 삭제**한다
- 재생목록은 업로드 스크립트가 넣어주지 않아 `playlistItems().insert` 로 직접 추가했다
- 썸네일은 API 의 `maxres` 존재만 믿지 않고 **실제로 내려받아 눈으로 확인**했다
  (훅·제목·저자·태그 합성 정상)

## 2026-09-12

### YouTube 업로드 완료
- 업로드된 책: Your_Paradise
- 업로드된 영상 수: 1개
- [1] [핵심 요약] 당신들의 천국: 이청준 (Your Paradise · AI 심층 분석)
  - URL: https://www.youtube.com/watch?v=72vRukqX0Cw

## 2026-09-12

### YouTube 업로드 완료
- 업로드된 책: Silent_Spring
- 업로드된 영상 수: 1개
- [1] [핵심 요약] 침묵의 봄: 레이첼 카슨 (Silent Spring · AI 심층 분석)
  - URL: https://www.youtube.com/watch?v=lVVu_nlKd5Q

## 2026-09-13

### YouTube 업로드 완료
- 업로드된 책: The_Tale_of_Unyeong
- 업로드된 영상 수: 1개
- [1] [핵심 요약] 운영전: 작자 미상 (The Tale of Unyeong · AI 심층 분석)
  - URL: https://www.youtube.com/watch?v=0eWjThwdbd4
...
- 업로드된 책: Aunt_Suni
- 업로드된 영상 수: 1개
- [1] [핵심 요약] 순이 삼촌: 현기영 (Aunt Suni · AI 심층 분석)
  - URL: https://www.youtube.com/watch?v=AU6rHiUO1gk

## 2026-09-21

### YouTube 업로드 완료
- 업로드된 책: The_Tale_of_a_Yangban
- 업로드된 영상 수: 1개
- [1] [핵심 요약] 양반전: 박지원 (The Tale of a Yangban · AI 심층 분석)
  - URL: https://www.youtube.com/watch?v=iyR04wtaFYE

## 2026-09-21

### YouTube 업로드 완료
- 업로드된 책: The_Old_Jar_Craftsman
- 업로드된 영상 수: 1개
- [1] [핵심 요약] 독 짓는 늙은이: 황순원 (The Old Jar Craftsman · AI 심층 분석)
  - URL: https://www.youtube.com/watch?v=uA7G9R_f6vA

## 2026-09-21

### YouTube 업로드 완료
- 업로드된 책: The_Tale_of_Lady_Pak
- 업로드된 영상 수: 1개
- [1] [핵심 요약] 박씨전: 작자 미상 (The Tale of Lady Pak · AI 심층 분석)
  - URL: https://www.youtube.com/watch?v=0Hx7sXbE15A

## 2026-09-22

### YouTube 업로드 완료
- 업로드된 책: The_Tale_of_Ong_Gojip
- 업로드된 영상 수: 1개
- [1] [핵심 요약] 옹고집전: 작자 미상 (The Tale of Ong Gojip · AI 심층 분석)
  - URL: https://www.youtube.com/watch?v=HT8LTkdY0Xg

## 2026-09-23

### YouTube 업로드 완료
- 업로드된 책: The_Tale_of_Choe_Cheok
- 업로드된 영상 수: 1개
- [1] [핵심 요약] 최척전: 조위한 (The Tale of Choe Cheok · AI 심층 분석)
  - URL: https://www.youtube.com/watch?v=aaNcU303Vpc

## 2026-09-24

### YouTube 업로드 완료
- 업로드된 책: The_Tale_of_Hong_Gyewol
- 업로드된 영상 수: 1개
- [1] [핵심 요약] 홍계월전: 작자 미상 (The Tale of Hong Gyewol · AI 심층 분석)
  - URL: https://www.youtube.com/watch?v=1vrHBnsBR6Y

## 2026-09-24

### YouTube 업로드 완료
- 업로드된 책: The_Tale_of_Manboksa_Temple
- 업로드된 영상 수: 1개
- [1] [핵심 요약] 만복사저포기: 김시습 (The Tale of Manboksa Temple · AI 심층 분석)
  - URL: https://www.youtube.com/watch?v=1qg5k1ZlqA4

## 2026-09-24

### YouTube 업로드 완료
- 업로드된 책: The_Tale_of_Jeong_Sujeong
- 업로드된 영상 수: 1개
- [1] [핵심 요약] 정수정전: 작자 미상 (The Tale of Jeong Sujeong · AI 심층 분석)
  - URL: https://www.youtube.com/watch?v=DlWPiyGZ0nk

## 2026-09-26

### YouTube 업로드 완료
- 업로드된 책: Tale_of_the_Rabbit
- 업로드된 영상 수: 1개
- [1] [핵심 요약] 토끼전: 작자 미상 (Tale of the Rabbit · AI 심층 분석)
  - URL: https://www.youtube.com/watch?v=PN3T6uT3hXA

## 2026-09-26

### YouTube 업로드 완료
- 업로드된 책: The_Tale_of_the_Golden_Bell
- 업로드된 영상 수: 1개
- [1] [핵심 요약] 금방울전: 작자 미상 (The Tale of the Golden Bell · AI 심층 분석)
  - URL: https://www.youtube.com/watch?v=wbXr6yPuZiE
