...
## ⚡ 다음 작업 세션 체크리스트 (2026-09-04 기준)

> ✅ **미완 작업 없음.** 실행 중인 렌더·NLM·업로드 없고 워킹 트리 깨끗(`352f439` 푸시 완료).

### 채널 현황
- 채널 영상 **484편**. 예약 큐 **21편**, 꼬리 **11/09 동물농장**. 다음 빈 슬롯은 **10/02**
  (근접 슬롯 삽입 방식이면 그 앞 아무 날짜 + 이후 13편 +3일)

### 새 선정 기준으로 만든 8편 (전부 예약 완료)
| 공개일 | 책 | 경쟁 | 영상 ID |
|---|---|---:|---|
| 9/07 | 만세전 (염상섭) | 2,632 | `YhBewX4kj-g` |
| 9/10 | 우상의 눈물 (전상국) | 1,408 | `dqc8CnTIEbc` |
| 9/14 | 수난이대 (하근찬) | 747 | `hsItetT2Ntk` |
| 9/17 | 유자소전 (이문구) | 950 | `23WgiKeuLsQ` |
| 9/20 | 만무방 (김유정) | **499** | `41uMkywCzGQ` |
| 9/23 | 기억 속의 들꽃 (윤흥길) | 1,496 | `KeWoDPb-Cd8` |
| 9/26 | 레디메이드 인생 (채만식) | 1,119 | `8UA95iAOJ48` |
| 9/29 | 니코마코스 윤리학 (아리스토텔레스) | 872 | `qhsDCfqoXJc` |

### 다음 우선 실행 액션
1. ⛔ **미리 쌓지 않는다** — 기본은 대기. 공개 반응(9/07부터 순차 공개)을 보고 판단
2. 다음 편 후보는 `data/candidates.md` — 원미동 사람들(2,099) · 이중나선(752) ·
   당신들의 천국(817) · B사감과 러브레터(849) · 논 이야기(1,037) · 미스터 방(4,497)
3. **선정 기준 실험 판독**: 경쟁 낮음 3편(8/27·8/31·9/03) vs 높음 3연작.
   ⚠️ 30일이 아니라 **60~90일**(11월 전후)

### 제작 시 반드시 지킬 것 (최근 사고에서 도출)
- **렌더 전 `scripts/check_image_brightness.py` 실행.** 판정 기준:
  평균밝기가 낮아도 **σ>20 이고 피사체가 읽히면 유지**. 한 자릿수 평균 + 낮은 σ만 제거
...
- **TTS 길이를 미리 계산**: echo 7.4자/초, nova 5.5자/초. 음성은 랜덤이라
  1,800~1,950자면 5분대에 들어온다. 필요 이미지 = TTS초 ÷ 4.5
